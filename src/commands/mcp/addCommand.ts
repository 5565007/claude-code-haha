  ...
  mcp
    ...
    .description(
      'Add an MCP server to Claude Code.\n\n' +
        ...
        '  rosecc mcp add --transport http corridor https://app.corridor.dev/api/mcp --header "Authorization: Bearer ..."\n\n' +
        '  # Add stdio server with environment variables:\n' +
        '  rosecc mcp add -e API_KEY=xxx my-server -- npx my-mcp-server\n\n' +
        '  # Add stdio server with subprocess flags:\n' +
        '  rosecc mcp add my-server -- my-command --some-flag arg1',
    )
    .option(
      '-s, --scope <scope>',
      ...
      if (!name) {
        cliError(
          'Error: Server name is required.\n' +
            'Usage: rosecc mcp add <name> <command> [args...]',
        )
      ...
      try {
        ...
        if (transport === 'sse') {
          ...
          if (!transportExplicit && looksLikeUrl) {
            ...
            process.stderr.write(
              `If this is an HTTP server, use: rosecc mcp add --transport http ${name} ${actualCommand}\n`,
            )
            process.stderr.write(
              `If this is an SSE server, use: rosecc mcp add --transport sse ${name} ${actualCommand}\n`,
            )
          }
